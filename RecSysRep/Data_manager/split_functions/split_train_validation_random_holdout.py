...
def split_train_in_two_percentage_user_selected(URM_train, train_percentage = 0.1, n_groups = 1, verbose = False, seed=1234):
    """
    The function splits an URM in n_groups pair of matrices (+1 if n_groups cannot divide n_users) selecting the number of interactions one user at a time
    :param URM_train:
    :param train_percentage:
    :param n_groups:
    :param verbose:
    ...
    num_users_per_group = num_users // n_groups
    num_residual = num_users - num_users_per_group * n_groups

    URM_train_builders = [IncrementalSparseMatrix(n_rows=num_users_per_group, n_cols=num_items, auto_create_col_mapper=False, auto_create_row_mapper=False) for i in n_groups]
    URM_validation_builders = [IncrementalSparseMatrix(n_rows=num_users_per_group, n_cols=num_items, auto_create_col_mapper=False, auto_create_row_mapper=False) for i in n_groups]
    if num_residual != 0:
        URM_train_builders.append(IncrementalSparseMatrix(n_rows=num_residual, n_cols=num_items, auto_create_col_mapper=False, auto_create_row_mapper=False))
        URM_validation_builders.append(IncrementalSparseMatrix(n_rows=num_residual, n_cols=num_items, auto_create_col_mapper=False, auto_create_row_mapper=False))
    ...
    for user_id in range(URM_train.shape[0]):
        ...
        URM_train_builders[matrix_index].add_data_lists([user_id]*len(train_items), train_items, train_ratings)
        URM_validation_builders[matrix_index].add_data_lists([user_id]*len(validation_items), validation_items, validation_ratings)

        user_considered += 1
        if user_considered == num_users_per_group:
            matrix_index += 1
    ...
    URM_train = [matr.get_SparseMatrix() for matr in URM_train_builders]
    URM_validation = [matr.get_SparseMatrix() for matr in URM_validation_builders]

    return URM_train, URM_validation

def split_train_in_two_percentage_global_sample(URM_all, train_percentage = 0.1, seed = 1234):
    """
    The function splits an URM in two matrices selecting the number of interactions globally, and setting a seed
    :param URM_all:
    :param train_percentage:
    :param verbose:
    :param seed:
